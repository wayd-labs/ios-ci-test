...
xcodebuild -exportArchive -exportFormat IPA -archivePath "$ARCHIVE_PATH" -exportPath "$APP_DIR/$APPNAME.ipa" 
...
$CRASHLYTICS/submit "$CRASHLYTICS_API" "$CRASHLYTICS_SECRET" -ipaPath "$APP_DIR/$APPNAME.ipa" -emails "$MAIL_LIST" -notifications YES
